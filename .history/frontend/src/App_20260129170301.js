        ...
        <AnimatePresence mode="wait">
          {activeTab === 'uni' && (
            <motion.div key="uni" className="projects-list" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
              {[
                { title: 'GesstiFest', course: 'Interacción Persona-Ordenador', tech: 'WPF, GUI, usabilidad', highlight: false },
                { title: 'Lab_C1_SanchezPalomino', course: 'Arquitectura de Computadores', tech: 'OpenMP, DPC, Hiperespectral', highlight: false },
                { title: '2024-ISO2-BC5', course: 'Ingeniería de Software 2', tech: 'PUD, GitHub, Maven, MySQL, POI', highlight: false },
                { title: 'remote-types', course: 'Sistemas Distribuidos', tech: 'ZeroC Ice, JSON, Apache Kafka', highlight: false },
                { title: 'Sokoban_SSII', course: 'Sistemas Inteligentes', tech: 'Algoritmos de búsqueda, Python', highlight: false },
                { title: 'Digital Builders 2025 — ESI & NTT DATA (3º Puesto)', course: 'Proyecto universitario Scrum', tech: 'Metodología ágil, MVP, stakeholders', highlight: true },
                { title: 'Proyecto Completo Gestión Proyectos', course: 'Gestión Integral de Proyectos Software', tech: 'Scrum, planificación, control de calidad', highlight: false }
              ].map((project, i) => (
                <motion.div key={i} className={`project-card ${project.highlight ? 'highlight' : ''}`} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}>
                  ...
                  <a href="https://github.com/SantiagoSanchez-CeleminArevalo/Portfolio_SantiagoSanchez-CeleminArevalo/blob/projects/projects/" target="_blank" rel="noopener noreferrer" className="github-link">
                    <Github className="w-4 h-4" /> Ver en GitHub <ExternalLink className="w-3 h-3" />
                  </a>
                </motion.div>
              ))}
            </motion.div>
